...
#if defined(__ANDROID__)
std::atomic<int> g_android_nce_memory_trace_budget{256};

void AndroidNceMemLogf(const char* fmt, ...) {
    if (g_android_nce_memory_trace_budget.fetch_sub(1, std::memory_order_relaxed) <= 0) {
        return;
    }
    va_list args;
    va_start(args, fmt);
    __android_log_vprint(ANDROID_LOG_ERROR, "NxEmuNCEMem", fmt ? fmt : "", args);
    va_end(args);
}
#else
void AndroidNceMemLogf(const char* fmt, ...) {
...
struct Memory::Impl {
    ...
    bool WriteExclusive128(Common::ProcessAddress vaddr, const u128 data, const u128 expected)
    {
        ...
        if (ptr)
        {
            ...
            // that are only 8-byte aligned. Switch code commonly uses LDXP/STXP pairs for
            // 128-bit exclusive sequences; emulating the final compare/store under a mutex
            // avoids endless STXP-failure retry loops while preserving guest-visible semantics.
            static std::mutex compare_exchange_128_mutex;
            std::scoped_lock lk{compare_exchange_128_mutex};
            u128 actual{};
            std::memcpy(actual.data(), ptr, sizeof(actual));
            if (actual != expected)
            {
                static int mismatch_log_count = 0;
                if (mismatch_log_count++ < 20)
                {
    ...
    void InvalidateGPUMemory(u8 * p, size_t size)
    {
        ...
        system.GetVideo().InvalidateGPUMemory(p, size);
    }
...
bool Memory::WriteBlock(uint64_t dest_addr, const void* src_buffer, uint64_t size) {
    return WriteBlock(Common::ProcessAddress{dest_addr}, src_buffer, static_cast<std::size_t>(size));
}
...
bool Memory::InvalidateNCE(Common::ProcessAddress vaddr, size_t size) {
    ...
    u8* const ptr = impl->GetPointerImpl(
        raw_vaddr,
        [&] {
            AndroidNceMemLogf("InvalidateNCE unmapped callback vaddr=0x%llx size=0x%zx",
                              static_cast<unsigned long long>(raw_vaddr), size);
            LOG_ERROR(HW_Memory, "Unmapped InvalidateNCE for {} bytes @ {:#x}", size, raw_vaddr);
            mapped = false;
        },
        [&] {
            AndroidNceMemLogf("InvalidateNCE rasterizer callback vaddr=0x%llx size=0x%zx",
                              static_cast<unsigned long long>(raw_vaddr), size);
            rasterizer = true;
        });
    AndroidNceMemLogf("InvalidateNCE after GetPointer ptr=%p mapped=%d rasterizer=%d",
                      ptr, mapped ? 1 : 0, rasterizer ? 1 : 0);
    if (rasterizer) {
        AndroidNceMemLogf("InvalidateNCE before InvalidateGPUMemory ptr=%p size=0x%zx", ptr, size);
        impl->InvalidateGPUMemory(ptr, size);
        AndroidNceMemLogf("InvalidateNCE after InvalidateGPUMemory");
    }

#ifdef __linux__
    if (!rasterizer && mapped) {
        AndroidNceMemLogf("InvalidateNCE before DeferredMapSeparateHeap vaddr=0x%llx",
                          static_cast<unsigned long long>(raw_vaddr));
        const bool deferred_mapped = impl->buffer->DeferredMapSeparateHeap(raw_vaddr);
        AndroidNceMemLogf("InvalidateNCE after DeferredMapSeparateHeap result=%d",
                          deferred_mapped ? 1 : 0);
    }
#endif

    const bool result = mapped && ptr != nullptr;
    AndroidNceMemLogf("InvalidateNCE exit result=%d", result ? 1 : 0);
    return result;
}

bool Memory::InvalidateNCE(uint64_t vaddr, uint64_t size) {
    AndroidNceMemLogf("InvalidateNCE public enter this=%p vaddr=0x%llx size=0x%llx",
                      this, static_cast<unsigned long long>(vaddr),
                      static_cast<unsigned long long>(size));
    const bool result = InvalidateNCE(Common::ProcessAddress{vaddr}, static_cast<size_t>(size));
    AndroidNceMemLogf("InvalidateNCE public exit result=%d", result ? 1 : 0);
    return result;
}
