...
NvResult nvhost_ctrl::IocCtrlEventWait(IocCtrlEventWaitParams& params, bool is_allocation) {
    ...
    NxemuAndroidDiagnostics::RecordEvent(
        "NVDRV.nvhost_ctrl.EventWait.Enter",
        "syncpt=" + std::to_string(params.fence.id) +
            " threshold=" + std::to_string(params.fence.value) +
            " timeout=" + std::to_string(params.timeout) +
            " allocation=" + (is_allocation ? std::string{"true"} : std::string{"false"}) +
            " value_raw=" + std::to_string(params.value.raw));
    ...
    if (fence_id >= MaxSyncPoints) {
        NxemuAndroidDiagnostics::RecordEvent("NVDRV.nvhost_ctrl.EventWait.Result",
                                             "bad-syncpt syncpt=" + std::to_string(fence_id));
    ...
    if (params.fence.value == 0) {
        ...
        NxemuAndroidDiagnostics::RecordEvent(
            "NVDRV.nvhost_ctrl.EventWait.Result",
            "zero-threshold syncpt=" + std::to_string(fence_id) +
                " value_raw=" + std::to_string(params.value.raw));
    ...
    if (syncpoint_manager.IsFenceSignalled(params.fence)) {
        ...
        NxemuAndroidDiagnostics::RecordEvent(
            "NVDRV.nvhost_ctrl.EventWait.Result",
            "already-signalled syncpt=" + std::to_string(fence_id) +
                " threshold=" + std::to_string(params.fence.value) +
                " value_raw=" + std::to_string(params.value.raw));
        return NvResult::Success;
    }
    ...
    if (const auto new_value = syncpoint_manager.UpdateMin(fence_id);
        ...
        NxemuAndroidDiagnostics::RecordEvent(
            "NVDRV.nvhost_ctrl.EventWait.Result",
            "signalled-after-update syncpt=" + std::to_string(fence_id) +
                " threshold=" + std::to_string(params.fence.value) +
                " value_raw=" + std::to_string(params.value.raw));
        return NvResult::Success;
    }
    ...
    const auto check_failing = [&]() {
        if (events[slot].fails > 2) {
            NxemuAndroidDiagnostics::RecordEvent(
                "NVDRV.nvhost_ctrl.EventWait.ForceWaitHost",
                "slot=" + std::to_string(slot) + " syncpt=" + std::to_string(fence_id) +
                    " target=" + std::to_string(target_value) +
                    " fails=" + std::to_string(events[slot].fails));
            {
    ...
    if (slot >= MaxNvEvents) {
        NxemuAndroidDiagnostics::RecordEvent(
            "NVDRV.nvhost_ctrl.EventWait.Result",
            "bad-slot slot=" + std::to_string(slot) + " syncpt=" + std::to_string(fence_id));
        return NvResult::BadParameter;
    }
    ...
    if (params.timeout == 0) {
        if (check_failing()) {
            ...
            NxemuAndroidDiagnostics::RecordEvent(
                "NVDRV.nvhost_ctrl.EventWait.Result",
                "poll-success-after-force slot=" + std::to_string(slot) +
                    " syncpt=" + std::to_string(fence_id) +
                    " target=" + std::to_string(target_value));
            return NvResult::Success;
        }
        NxemuAndroidDiagnostics::RecordEvent(
            "NVDRV.nvhost_ctrl.EventWait.Result",
            "poll-timeout slot=" + std::to_string(slot) + " syncpt=" +
                std::to_string(fence_id) + " target=" + std::to_string(target_value) +
                " fails=" + std::to_string(events[slot].fails));
    ...
    if (!event.registered) {
        NxemuAndroidDiagnostics::RecordEvent(
            "NVDRV.nvhost_ctrl.EventWait.Result",
            "unregistered-event slot=" + std::to_string(slot) +
                " syncpt=" + std::to_string(fence_id));
        return NvResult::BadParameter;
    }
    ...
    if (event.IsBeingUsed()) {
        NxemuAndroidDiagnostics::RecordEvent(
            "NVDRV.nvhost_ctrl.EventWait.Result",
            "busy-event slot=" + std::to_string(slot) + " syncpt=" +
                std::to_string(fence_id) + " target=" + std::to_string(target_value));
        return NvResult::BadParameter;
    }
    ...
    if (check_failing()) {
        ...
        NxemuAndroidDiagnostics::RecordEvent(
            "NVDRV.nvhost_ctrl.EventWait.Result",
            "wait-success-after-force slot=" + std::to_string(slot) +
                " syncpt=" + std::to_string(fence_id) +
                " target=" + std::to_string(target_value));
        return NvResult::Success;
    }
    ...
    NxemuAndroidDiagnostics::RecordEvent(
        "NVDRV.nvhost_ctrl.EventWait.Result",
        "armed slot=" + std::to_string(slot) + " syncpt=" + std::to_string(fence_id) +
            " target=" + std::to_string(target_value) +
            " handle=" + std::to_string(event.wait_handle) +
            " value_raw=" + std::to_string(params.value.raw));
    return NvResult::Timeout;
}
...
NvResult nvhost_ctrl::IocCtrlEventRegister(IocCtrlEventRegisterParams& params) {
    ...
    NxemuAndroidDiagnostics::RecordEvent("NVDRV.nvhost_ctrl.EventRegister",
...
NvResult nvhost_ctrl::IocCtrlEventUnregister(IocCtrlEventUnregisterParams& params) {
    ...
    LOG_DEBUG(Service_NVDRV, " called, user_event_id: {:X}", event_id);
    NxemuAndroidDiagnostics::RecordEvent("NVDRV.nvhost_ctrl.EventUnregister",
                                         "event_id=" + std::to_string(event_id));
...
NvResult nvhost_ctrl::IocCtrlClearEventWait(IocCtrlEventClearParams& params) {
    ...
    NxemuAndroidDiagnostics::RecordEvent("NVDRV.nvhost_ctrl.ClearEventWait.Enter",
                                         "slot=" + std::to_string(event_id));
    ...
    if (event.status.exchange(EventState::Cancelling, std::memory_order_acq_rel) ==
        EventState::Waiting) {
        NxemuAndroidDiagnostics::RecordEvent(
            "NVDRV.nvhost_ctrl.ClearEventWait.Cancel",
            "slot=" + std::to_string(event_id) +
                " syncpt=" + std::to_string(event.assigned_syncpt) +
                " target=" + std::to_string(event.assigned_value) +
                " handle=" + std::to_string(event.wait_handle));
    ...
    event.kevent->Clear();
    NxemuAndroidDiagnostics::RecordEvent(
        "NVDRV.nvhost_ctrl.ClearEventWait.Result",
        "slot=" + std::to_string(event_id) + " fails=" + std::to_string(event.fails));
...
void nvhost_ctrl::HostActionCallback(uint32_t slot, void * userData)
{
    ...
    NxemuAndroidDiagnostics::RecordEvent(
        "NVDRV.nvhost_ctrl.HostAction",
        "slot=" + std::to_string(slot) +
            " syncpt=" + std::to_string(event_.assigned_syncpt) +
            " target=" + std::to_string(event_.assigned_value));
