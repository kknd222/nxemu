...
NvResult nvhost_as_gpu::AllocAsEx(IoctlAllocAsEx& params) {
    ...
    NxemuAndroidDiagnostics::RecordEvent(
        "NVDRV.nvhost_as_gpu.AllocAsEx",
        "big_page_size=" + std::to_string(params.big_page_size));
...
NvResult nvhost_as_gpu::MapBufferEx(IoctlMapBufferEx & params)
{
    ...
    NxemuAndroidDiagnostics::RecordEvent(
        "NVDRV.nvhost_as_gpu.MapBufferEx",
        "flags=" + std::to_string(static_cast<u32>(params.flags)) + " handle=" + std::to_string(params.handle) +
            " size=" + std::to_string(params.mapping_size) +
            " offset=0x" + fmt::format("{:X}", static_cast<s64>(params.offset)) +
            " buffer_offset=" + std::to_string(static_cast<s64>(params.buffer_offset)) +
            " kind=" + std::to_string(params.kind));
    ...
    if ((params.flags & MappingFlags::Remap) != MappingFlags::None) 
    {
        try 
        {
            ...
            NxemuAndroidDiagnostics::RecordEvent(
                "NVDRV.nvhost_as_gpu.MapBufferExResult",
                "mode=remap gpu=0x" + fmt::format("{:X}", gpu_address) +
                    " device=0x" + fmt::format("{:X}", static_cast<u64>(device_address)) +
                    " size=" + std::to_string(params.mapping_size));
            return NvResult::Success;
        } 
        catch (const std::out_of_range&) 
    ...
    if ((params.flags & MappingFlags::Fixed) != MappingFlags::None)
    {
        ...
        NxemuAndroidDiagnostics::RecordEvent(
            "NVDRV.nvhost_as_gpu.MapBufferExResult",
            "mode=fixed gpu=0x" + fmt::format("{:X}", static_cast<u64>(params.offset)) +
                " device=0x" + fmt::format("{:X}", static_cast<u64>(device_address)) +
                " size=" + std::to_string(size) +
                " big=" + (use_big_pages ? std::string{"true"} : std::string{"false"}));
    ...
    else
    {
        ...
        system.GetVideo().MapBufferEx(gmmu, params.offset, device_address, aligned_size, params.kind, big_page);
        NxemuAndroidDiagnostics::RecordEvent(
            "NVDRV.nvhost_as_gpu.MapBufferExResult",
            "mode=alloc gpu=0x" + fmt::format("{:X}", static_cast<u64>(params.offset)) +
                " device=0x" + fmt::format("{:X}", static_cast<u64>(device_address)) +
                " size=" + std::to_string(size) +
                " aligned=" + std::to_string(aligned_size) +
                " big=" + (big_page ? std::string{"true"} : std::string{"false"}));
...
{
    LOG_INFO(Service_NVDRV, "called, fd={:X}, gmmu={:X}", params.fd, gmmu);
    NxemuAndroidDiagnostics::RecordEvent("NVDRV.nvhost_as_gpu.BindChannel",
                                         "fd=" + std::to_string(params.fd) +
                                             " gmmu=" + std::to_string(gmmu));
