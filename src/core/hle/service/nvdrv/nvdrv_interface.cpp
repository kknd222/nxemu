...
void NVDRV::Open(HLERequestContext& ctx) {
    ...
    NxemuAndroidDiagnostics::RecordEvent("NVDRV.Open", "device=" + device_name);
    ...
    NxemuAndroidDiagnostics::RecordEvent("NVDRV.OpenResult",
                                         "device=" + device_name + " fd=" + std::to_string(fd));
...
void NVDRV::Ioctl1(HLERequestContext& ctx) {
    ...
    NxemuAndroidDiagnostics::RecordEvent(
        "NVDRV.Ioctl1",
        "fd=" + std::to_string(fd) + " ioctl=0x" + fmt::format("{:08X}", command.raw));
    ...
    NxemuAndroidDiagnostics::RecordEvent(
        "NVDRV.Ioctl1Result",
        "fd=" + std::to_string(fd) + " ioctl=0x" +
            fmt::format("{:08X}", command.raw) + " result=" +
            std::to_string(static_cast<u32>(nv_result)));
    if (command.is_out != 0) {
...
void NVDRV::Initialize(HLERequestContext& ctx) {
    ...
    NxemuAndroidDiagnostics::RecordEvent("NVDRV.Initialize", "called");
    IPC::ResponseBuilder rb{ctx, 3};
    SCOPE_EXIT {
        rb.Push(ResultSuccess);
    ...
    is_initialized = true;
    NxemuAndroidDiagnostics::RecordEvent("NVDRV.InitializeResult", "success");
}
...
void NVDRV::QueryEvent(HLERequestContext& ctx) {
    ...
    NxemuAndroidDiagnostics::RecordEvent("NVDRV.QueryEvent",
                                         "fd=" + std::to_string(fd) +
...
void NVDRV::SetAruid(HLERequestContext& ctx) {
    ...
    NxemuAndroidDiagnostics::RecordEvent("NVDRV.SetAruid", "pid=" + std::to_string(pid));
...
void NVDRV::SetGraphicsFirmwareMemoryMarginEnabled(HLERequestContext& ctx) {
    LOG_WARNING(Service_NVDRV, "(STUBBED) called");
    NxemuAndroidDiagnostics::RecordEvent("NVDRV.SetGraphicsFirmwareMemoryMarginEnabled", "called");
