...
Result SM::GetServiceImpl(Kernel::KClientSession** out_client_session, HLERequestContext& ctx) {
    ...
    Service::NxemuAndroidDiagnostics::RecordEvent("SM.GetService", "name=" + name);
    ...
    if (port_result != ResultSuccess) {
        Service::NxemuAndroidDiagnostics::RecordEvent(
            "SM.GetServiceResult",
            "name=" + name + " result=not_registered raw=0x" +
                fmt::format("{:08X}", port_result.raw));
    ...
    if (const auto result = client_port->CreateSession(&session); result.IsError()) {
        Service::NxemuAndroidDiagnostics::RecordEvent(
            "SM.GetServiceResult",
            "name=" + name + " result=create_session_error raw=0x" +
                fmt::format("{:08X}", result.raw));
        LOG_ERROR(Service_SM, "called service={} -> error 0x{:08X}", name, result.raw);
        return result;
    }

    *out_client_session = session;
    Service::NxemuAndroidDiagnostics::RecordEvent("SM.GetServiceResult",
