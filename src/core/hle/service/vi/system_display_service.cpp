...
Result ISystemDisplayService::SetLayerVisibility(bool visible, u64 layer_id)
{
    ...
    NxemuAndroidDiagnostics::RecordEvent(
        "VI.System.SetLayerVisibility",
        "layer_id=" + std::to_string(layer_id) + " visible=" + (visible ? "true" : "false"));
    R_SUCCEED();
}
...
Result ISystemDisplayService::GetSharedBufferMemoryHandleId(Out<s32> out_nvmap_handle, Out<u64> out_size, OutLargeData<SharedMemoryPoolLayout, BufferAttr_HipcMapAlias> out_pool_layout, u64 buffer_id, ClientAppletResourceUserId aruid)
{
    ...
    NxemuAndroidDiagnostics::RecordEvent(
        "VI.System.GetSharedBufferMemoryHandleId",
        "buffer_id=" + std::to_string(buffer_id) + " aruid=" + std::to_string(aruid.pid));
...
Result ISystemDisplayService::OpenSharedLayer(u64 layer_id)
{
    ...
    NxemuAndroidDiagnostics::RecordEvent("VI.System.OpenSharedLayer",
...
{
    LOG_INFO(Service_VI, "(STUBBED) called. layer_id={}", layer_id);
    NxemuAndroidDiagnostics::RecordEvent("VI.System.ConnectSharedLayer",
                                         "layer_id=" + std::to_string(layer_id));
    R_SUCCEED();
}
                                                       ...
                                                       Out<s64> out_target_slot, u64 layer_id)
{
    ...
    NxemuAndroidDiagnostics::RecordEvent("VI.System.AcquireSharedFrameBuffer",
                                         "layer_id=" + std::to_string(layer_id));
...
{
    LOG_DEBUG(Service_VI, "called, layer_id={}, surface_id={}", layer_id, surface_id);
    NxemuAndroidDiagnostics::RecordEvent(
        "VI.System.PresentSharedFrameBuffer",
        "layer_id=" + std::to_string(layer_id) + " surface_id=" +
            std::to_string(surface_id) + " swap_interval=" + std::to_string(swap_interval));
    ...
    OutCopyHandle<Kernel::KReadableEvent> out_event, u64 layer_id)
{
    ...
    NxemuAndroidDiagnostics::RecordEvent("VI.System.GetSharedFrameBufferAcquirableEvent",
                                         "layer_id=" + std::to_string(layer_id));
