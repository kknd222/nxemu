...
IDeliveryCacheStorageService::IDeliveryCacheStorageService(Core::System& system_) :
    ServiceFramework{system_, "IDeliveryCacheStorageService"}
{
    // clang-format off
...
Result IDeliveryCacheStorageService::EnumerateDeliveryCacheDirectory(Out<s32> out_directory_count, OutArray<DirectoryName, BufferAttr_HipcMapAlias> out_directories)
{
    ...
    *out_directory_count = static_cast<s32>(std::min(out_directories.size(), entries.size() - next_read_index));
    memcpy(out_directories.data(), entries.data() + next_read_index, *out_directory_count * sizeof(DirectoryName));
