...
void SurfaceFlinger::AddDisplay(u64 display_id) {
    ...
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "Nvnflinger.SurfaceFlinger.AddDisplay", "display_id=" + std::to_string(display_id));
}
...
bool SurfaceFlinger::ComposeDisplay(s32* out_swap_interval, f32* out_compose_speed_scale,
    ...
    if (!display) {
        Service::NxemuAndroidDiagnostics::RecordEvent(
            "Nvnflinger.SurfaceFlinger.ComposeDisplay",
            "display_id=" + std::to_string(display_id) + " result=no_display");
        return false;
    }

    if (!display->stack.HasLayers()) {
        static u64 no_layer_counter{};
        ++no_layer_counter;
        if (no_layer_counter <= 20 || (no_layer_counter % 300) == 0) {
            Service::NxemuAndroidDiagnostics::RecordEvent(
                "Nvnflinger.SurfaceFlinger.ComposeDisplay",
                "display_id=" + std::to_string(display_id) + " result=no_layers displays=" +
                    std::to_string(m_displays.size()));
        }
        return false;
    }
    ...
    *out_swap_interval =
    ...
    static u64 compose_counter{};
    ++compose_counter;
    if (compose_counter <= 20 || (compose_counter % 300) == 0) {
        Service::NxemuAndroidDiagnostics::RecordEvent(
            "Nvnflinger.SurfaceFlinger.ComposeDisplay",
            "display_id=" + std::to_string(display_id) + " result=composed layers=" +
                std::to_string(display->stack.layers.size()) +
                " swap_interval=" + std::to_string(*out_swap_interval));
    }
    return true;
}
...
void SurfaceFlinger::CreateLayer(s32 consumer_binder_id) {
    ...
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "Nvnflinger.SurfaceFlinger.CreateLayer",
        "consumer_binder_id=" + std::to_string(consumer_binder_id));
}
...
void SurfaceFlinger::AddLayerToDisplayStack(u64 display_id, s32 consumer_binder_id) {
    ...
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "Nvnflinger.SurfaceFlinger.AddLayerToDisplayStack",
        "display_id=" + std::to_string(display_id) +
            " consumer_binder_id=" + std::to_string(consumer_binder_id) +
            " layer_count=" + std::to_string(display->stack.layers.size()));
}
...
void SurfaceFlinger::CreateBufferQueue(s32* out_consumer_binder_id, s32* out_producer_binder_id) {
    ...
    *out_producer_binder_id = m_server.RegisterBinder(std::move(producer));
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "Nvnflinger.SurfaceFlinger.CreateBufferQueue",
        "consumer_binder_id=" + std::to_string(*out_consumer_binder_id) +
            " producer_binder_id=" + std::to_string(*out_producer_binder_id));
}
