...
void ServiceFrameworkBase::ReportUnimplementedFunction(HLERequestContext& ctx, const FunctionInfoBase* info)
{
    ...
    // Android PoC stability guard:
    // On desktop this assert is useful during development, but on Android a single missing HLE
    // command (for example IFileSystem::GetFileTimeStampRaw from some NSPs) aborts the whole APK
    // before diagnostics can be copied. Treat unknown service commands as auto-stub warnings when
    // auto-stub is enabled, and only assert in strict mode.
    ...
    {
        LOG_WARNING(Service, "Unknown / unimplemented {}", fmt::to_string(buf));
    }
    NxemuAndroidDiagnostics::RecordEvent(
        "IPC.Unimplemented",
        fmt::format("service={} cmd={} name={} type={} auto_stub={}", service_name,
                    ctx.GetCommand(), function_name, static_cast<u32>(ctx.GetCommandType()),
                    Settings::values.use_auto_stub.GetValue()));
    if (Settings::values.use_auto_stub)
    {
...
void ServiceFrameworkBase::InvokeRequest(HLERequestContext& ctx)
{
    ...
    NxemuAndroidDiagnostics::RecordEvent(
        "IPC.Call", fmt::format("service={} cmd={} name={}", service_name, ctx.GetCommand(),
                                info->name));
    LOG_TRACE(Service, "{}", MakeFunctionString(info->name, GetServiceName(), ctx.CommandBuffer()));
    handler_invoker(this, info->handler_callback, ctx);
    NxemuAndroidDiagnostics::RecordEvent(
        "IPC.Return", fmt::format("service={} cmd={} name={}", service_name, ctx.GetCommand(),
                                  info->name));
}
...
void ServiceFrameworkBase::InvokeRequestTipc(HLERequestContext& ctx)
{
    ...
    NxemuAndroidDiagnostics::RecordEvent(
        "TIPC.Call", fmt::format("service={} cmd={} name={}", service_name, ctx.GetCommand(),
                                info->name));
    LOG_TRACE(Service, "{}", MakeFunctionString(info->name, GetServiceName(), ctx.CommandBuffer()));
    handler_invoker(this, info->handler_callback, ctx);
    NxemuAndroidDiagnostics::RecordEvent(
        "TIPC.Return", fmt::format("service={} cmd={} name={}", service_name, ctx.GetCommand(),
                                  info->name));
}
...
Result ServiceFrameworkBase::HandleSyncRequest(Kernel::KServerSession& session,
    ...
    default:
        ...
        NxemuAndroidDiagnostics::RecordEvent(
            "IPC.UnknownCommandType",
            fmt::format("service={} command_type={} cmd={}", service_name,
                        static_cast<u32>(ctx.GetCommandType()), ctx.GetCommand()));
        break;
    }
