...
Result ISelfController::SetFocusHandlingMode(bool notify, bool background, bool suspend) {
    ...
    NxemuAndroidDiagnostics::RecordEvent(
        "AM.Self.SetFocusHandlingMode",
        std::string{"notify="} + (notify ? "true" : "false") +
...
Result ISelfController::SetOutOfFocusSuspendingEnabled(bool enabled) {
    LOG_INFO(Service_AM, "called, enabled={}", enabled);
    NxemuAndroidDiagnostics::RecordEvent("AM.Self.SetOutOfFocusSuspendingEnabled",
                                         std::string{"enabled="} +
                                             (enabled ? "true" : "false"));
...
Result ISelfController::IsSystemBufferSharingEnabled() {
    ...
    NxemuAndroidDiagnostics::RecordEvent("AM.Self.IsSystemBufferSharingEnabled", "called");
...
Result ISelfController::GetSystemSharedBufferHandle(Out<u64> out_buffer_id) {
    LOG_INFO(Service_AM, "called");
    NxemuAndroidDiagnostics::RecordEvent("AM.Self.GetSystemSharedBufferHandle", "called");
...
Result ISelfController::GetSystemSharedLayerHandle(Out<u64> out_buffer_id, Out<u64> out_layer_id) {
    LOG_INFO(Service_AM, "called");
    NxemuAndroidDiagnostics::RecordEvent("AM.Self.GetSystemSharedLayerHandle", "called");
...
Result ISelfController::CreateManagedDisplayLayer(Out<u64> out_layer_id) {
    LOG_INFO(Service_AM, "called");
    NxemuAndroidDiagnostics::RecordEvent("AM.Self.CreateManagedDisplayLayer", "called");
...
Result ISelfController::CreateManagedDisplaySeparableLayer(Out<u64> out_layer_id,
    ...
    LOG_WARNING(Service_AM, "(STUBBED) called");
    NxemuAndroidDiagnostics::RecordEvent("AM.Self.CreateManagedDisplaySeparableLayer", "called");
