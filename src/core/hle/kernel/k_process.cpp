...
void KProcess::LoadModule(const IModuleInfo & module, KProcessAddress base_addr)
{
    ...
    const bool android_nce_requested = g_settings->GetBool(NXCpuSetting::NceEnabled) ||
                                       g_settings->GetInt(NXCpuSetting::CpuBackend) == 1;
    if (android_nce_requested)
    {
        auto & buffer = m_kernel.System().DeviceMemory().buffer;
        const auto code_addr = GetInteger(base_addr + module.CodeSegmentAddr());
        const auto code_size = module.CodeSegmentSize();
        LOG_INFO(Kernel, "Android NCE protecting module code executable: app={} base={:#x} code={:#x}+{:#x} nce={} backend={}",
                 this->IsApplication(), GetInteger(base_addr), code_addr, code_size,
                 g_settings->GetBool(NXCpuSetting::NceEnabled),
                 g_settings->GetInt(NXCpuSetting::CpuBackend));
        buffer.Protect(code_addr, code_size,
                       Common::MemoryPermission::Read | Common::MemoryPermission::Execute);

        const auto patch_addr = module.PatchSegmentAddr();
        const auto patch_size = module.PatchSegmentSize();
        if (patch_size != 0)
        {
            buffer.Protect(GetInteger(base_addr + patch_addr), patch_size,
                           Common::MemoryPermission::Read | Common::MemoryPermission::Execute);
            m_page_table.SetProcessMemoryPermission(base_addr + patch_addr, patch_size,
                                                    Svc::MemoryPermission::None);
        }

        const uint32_t post_count = module.PatchPostHandlerCount();
        for (uint32_t i = 0; i < post_count; ++i)
        {
            uint64_t module_pc = 0;
            uint64_t patch_pc = 0;
            if (module.PatchPostHandler(i, &module_pc, &patch_pc))
            {
                m_post_handlers[module_pc] = patch_pc;
            }
        }
        if (post_count != 0)
        {
            LOG_INFO(Kernel,
                     "Android NCE registered post trampolines: base={:#x} count={} total={}",
                     GetInteger(base_addr), post_count, m_post_handlers.size());
        }
    }
#endif
}
