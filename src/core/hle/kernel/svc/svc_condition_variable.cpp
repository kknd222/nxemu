...
Result WaitProcessWideKeyAtomic(Core::System& system, u64 address, u64 cv_key, u32 tag,
    ...
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.WaitProcessWideKey",
        fmt::format("addr={:#x} key={:#x} tag={:#x} timeout_ns={}", address, cv_key, tag,
                    timeout_ns));
    ...
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.WaitProcessWideKeyResult",
        fmt::format("addr={:#x} key={:#x} tag={:#x} timeout_ns={} result={:#x}", address, cv_key,
                    tag, timeout_ns, result.GetInnerValue()));
    R_RETURN(result);
}
...
void SignalProcessWideKey(Core::System& system, u64 cv_key, s32 count) {
    ...
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.SignalProcessWideKey", fmt::format("key={:#x} count={}", cv_key, count));
