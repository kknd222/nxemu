...
Result MapPhysicalMemory(Core::System& system, u64 addr, u64 size) {
    ...
    const Result result = page_table.MapPhysicalMemory(addr, size);
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.MapPhysicalMemoryResult",
        fmt::format("addr={:#x} size={:#x} result={:#x} totalSys={:#x} usedSys={:#x}", addr, size,
                    result.GetInnerValue(), current_process->GetTotalSystemResourceSize(),
                    current_process->GetUsedSystemResourceSize()));
    R_RETURN(result);
}
...
Result UnmapPhysicalMemory(Core::System& system, u64 addr, u64 size) {
    ...
    const Result result = page_table.UnmapPhysicalMemory(addr, size);
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.UnmapPhysicalMemoryResult",
        fmt::format("addr={:#x} size={:#x} result={:#x} totalSys={:#x} usedSys={:#x}", addr, size,
                    result.GetInnerValue(), current_process->GetTotalSystemResourceSize(),
                    current_process->GetUsedSystemResourceSize()));
    R_RETURN(result);
