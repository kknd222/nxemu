...
Result ResetSignal(Core::System& system, Handle handle) {
    ...
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.ResetSignal", fmt::format("handle={:#x}", handle));
    ...
    // Try to reset as readable event.
    {
        ...
        if (readable_event.IsNotNull()) {
            auto result = readable_event->Reset();
            Service::NxemuAndroidDiagnostics::RecordEvent(
                "SVC.ResetSignalResult",
                fmt::format("handle={:#x} type=readable_event result={:#x}", handle,
    ...
    // Try to reset as process.
    {
        ...
        if (process.IsNotNull()) {
            auto result = process->Reset();
            Service::NxemuAndroidDiagnostics::RecordEvent(
                "SVC.ResetSignalResult",
                fmt::format("handle={:#x} type=process result={:#x}", handle,
                            result.GetInnerValue()));
            R_RETURN(result);
        }
    }
...
Result WaitSynchronization(Core::System& system, int32_t* out_index, u64 user_handles,
    ...
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.WaitSync",
        fmt::format("handles_ptr={:#x} num={} timeout_ns={}", user_handles, num_handles,
                    timeout_ns));
    ...
    if (num_handles > 0) {
        ...
        Service::NxemuAndroidDiagnostics::RecordEvent(
            "SVC.WaitSyncHandles",
            [&] {
                std::string detail = fmt::format("num={} values=", num_handles);
                for (auto i = 0; i < num_handles && i < 4; ++i) {
                    detail += fmt::format("{}{}", i ? "," : "", handles[i]);
                }
                return detail;
            }());
    ...
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.WaitSyncResult",
        fmt::format("result={:#x} out_index={} num={} timeout_ns={}", res.GetInnerValue(),
                    out_index != nullptr ? *out_index : -999, num_handles, timeout_ns));
...
Result CancelSynchronization(Core::System& system, Handle handle) {
    ...
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.CancelSync", fmt::format("handle={:#x}", handle));
