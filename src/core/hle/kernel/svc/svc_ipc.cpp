...
Result SendSyncRequest(Core::System& system, Handle session_handle) {
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.SendSyncRequest", fmt::format("handle={:#x}", session_handle));
    const Result result = SendSyncRequestImpl(system.Kernel(), 0, 0, session_handle);
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.SendSyncRequestResult",
        fmt::format("handle={:#x} result={:#x}", session_handle, result.GetInnerValue()));
    R_RETURN(result);
}
...
Result SendSyncRequestWithUserBuffer(Core::System& system, uint64_t message, uint64_t buffer_size,
                                     Handle session_handle) {
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.SendSyncRequestUB",
        fmt::format("handle={:#x} message={:#x} size={:#x}", session_handle, message,
                    buffer_size));
...

    {
        ...
        // Send the request.
        ASSERT(message != 0);
        const Result result = SendSyncRequestImpl(kernel, message, buffer_size, session_handle);
        Service::NxemuAndroidDiagnostics::RecordEvent(
            "SVC.SendSyncRequestUBResult",
            fmt::format("handle={:#x} result={:#x} message={:#x} size={:#x}", session_handle,
                        result.GetInnerValue(), message, buffer_size));
        R_TRY(result);
    }

    // We successfully processed, so try to unlock the message buffer.
    const Result unlock_result = page_table.UnlockForIpcUserBuffer(message, buffer_size);
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.SendSyncRequestUBUnlock",
        fmt::format("handle={:#x} result={:#x}", session_handle, unlock_result.GetInnerValue()));
    R_RETURN(unlock_result);
}
...
Result ReplyAndReceive(Core::System& system, s32* out_index, uint64_t handles, s32 num_handles,
    ...
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.ReplyAndReceive",
        fmt::format("handles_ptr={:#x} num={} reply={:#x} timeout_ns={}", handles, num_handles,
                    reply_target, timeout_ns));
    const Result result = ReplyAndReceiveImpl(system.Kernel(), out_index, 0, 0, 0, handles,
                                              num_handles, reply_target, timeout_ns);
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.ReplyAndReceiveResult",
        fmt::format("result={:#x} out_index={} num={} reply={:#x}", result.GetInnerValue(),
                    out_index != nullptr ? *out_index : -999, num_handles, reply_target));
    R_RETURN(result);
}
...
Result ReplyAndReceiveWithUserBuffer(Core::System& system, int32_t* out_index, uint64_t message,
    ...
    Service::NxemuAndroidDiagnostics::RecordEvent(
        "SVC.ReplyAndReceiveUB",
        fmt::format("message={:#x} size={:#x} handles_ptr={:#x} num={} reply={:#x} timeout_ns={}",
                    message, buffer_size, handles, num_handles, reply_target, timeout_ns));
    // Validate that the message buffer is page aligned and does not overflow.
    R_UNLESS(Common::IsAligned(message, PageSize), ResultInvalidAddress);
    R_UNLESS(buffer_size > 0, ResultInvalidSize);
    R_UNLESS(Common::IsAligned(buffer_size, PageSize), ResultInvalidSize);
...

    {
        ...
        ASSERT(message != 0);
        const Result result = ReplyAndReceiveImpl(system.Kernel(), out_index, message, buffer_size,
                                                  message_paddr, handles, num_handles,
                                                  reply_target, timeout_ns);
        Service::NxemuAndroidDiagnostics::RecordEvent(
            "SVC.ReplyAndReceiveUBResult",
            fmt::format("result={:#x} out_index={} num={} reply={:#x}", result.GetInnerValue(),
                        out_index != nullptr ? *out_index : -999, num_handles, reply_target));
        R_TRY(result);
    }
