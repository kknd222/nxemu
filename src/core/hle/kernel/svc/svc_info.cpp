...
Result GetInfo(Core::System& system, u64* result, InfoType info_id_type, Handle handle,
    ...
    const auto log_result = [&](Result rc, const char* note = "") {
        Service::NxemuAndroidDiagnostics::RecordEvent(
            "SVC.GetInfoResult",
            fmt::format("info={} sub={} handle={:#x} result={:#x} value={:#x} {}",
                        info_id, info_sub_id, handle, rc.GetInnerValue(),
                        result != nullptr ? *result : 0, note != nullptr ? note : ""));
        return rc;
    };
    ...
    case InfoType::AliasRegionExtraSize:
    {
        ...
        R_RETURN(log_result(ResultInvalidEnumValue, "unimplemented-inner"));
    }
    ...
    default:
        LOG_ERROR(Kernel_SVC, "Unimplemented svcGetInfo id=0x{:016X}", info_id);
        R_RETURN(log_result(ResultInvalidEnumValue, "unimplemented"));
    }
}
