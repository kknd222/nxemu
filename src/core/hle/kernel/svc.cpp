...
const char* SvcName(u32 imm) {
    ...
    case SvcId::GetThreadCoreMask:
        return "GetThreadCoreMask";
    case SvcId::SetThreadCoreMask:
        return "SetThreadCoreMask";
    ...
    case SvcId::UnmapPhysicalMemory:
        return "UnmapPhysicalMemory";
    default:
        return "other";
    }
}

bool ShouldLogSvc(u32 imm) {
    using namespace std::chrono;
    static std::array<steady_clock::time_point, 0x80> last{};
    static std::array<uint32_t, 0x80> skipped{};
    const auto now = steady_clock::now();
    const auto index = imm < last.size() ? imm : 0;
    const bool important = imm == static_cast<u32>(SvcId::WaitSynchronization) ||
                           imm == static_cast<u32>(SvcId::SendSyncRequest) ||
                           imm == static_cast<u32>(SvcId::SendSyncRequestWithUserBuffer) ||
                           imm == static_cast<u32>(SvcId::ReplyAndReceive) ||
                           imm == static_cast<u32>(SvcId::ReplyAndReceiveWithUserBuffer) ||
                           imm == static_cast<u32>(SvcId::WaitProcessWideKeyAtomic) ||
                           imm == static_cast<u32>(SvcId::ArbitrateLock) ||
                           imm == static_cast<u32>(SvcId::SleepThread) ||
                           imm == static_cast<u32>(SvcId::QueryMemory) ||
                           imm == static_cast<u32>(SvcId::GetInfo) ||
                           imm == static_cast<u32>(SvcId::MapPhysicalMemory);
    const auto interval = important ? 250ms : 1500ms;
    skipped[index]++;
    if (now - last[index] < interval) {
        return false;
    }
    last[index] = now;
    skipped[index] = 0;
...
void Call(Core::System& system, u32 imm) {
    ...
    if (log_svc) {
        ...
        Service::NxemuAndroidDiagnostics::RecordEvent(
            "SVC.Call",
            fmt::format("id={:#x} name={} tid={} pc={:#x} lr={:#x} a0={:#x} a1={:#x} a2={:#x} a3={:#x} a4={:#x} a5={:#x}",
                        imm, SvcName(imm), thread.GetThreadId(), ctx.pc, ctx.lr, args[0], args[1],
    ...
    if (log_svc) {
        ...
        Service::NxemuAndroidDiagnostics::RecordEvent(
            "SVC.Return",
            fmt::format("id={:#x} name={} tid={} pc={:#x} ret0={:#x} ret1={:#x}",
                        imm, SvcName(imm), thread.GetThreadId(), ctx.pc, args[0], args[1]));
    }
}
