...
VirtualFile VfsFilesystem::OpenFile(std::string_view path_, VirtualFileOpenMode perms) {
    const auto path = Common::FS::SanitizePath(path_);
    return root->GetFileRelative(path);
}

VirtualFile VfsFilesystem::CreateFile(std::string_view path_, VirtualFileOpenMode perms) {
    const auto path = Common::FS::SanitizePath(path_);
...
VirtualFile VfsFilesystem::CopyFile(std::string_view old_path_, std::string_view new_path_) {
    ...
    const auto old_file = OpenFile(old_path, VirtualFileOpenMode::Read);
    if (old_file == nullptr)
        return nullptr;
    auto new_file = OpenFile(new_path, VirtualFileOpenMode::Read);
    if (new_file != nullptr)
        return nullptr;
    new_file = CreateFile(new_path, VirtualFileOpenMode::Write);
    if (new_file == nullptr)
        return nullptr;
...
bool VfsFilesystem::DeleteFile(std::string_view path_) {
    const auto path = Common::FS::SanitizePath(path_);
    auto parent = OpenDirectory(Common::FS::GetParentPath(path), VirtualFileOpenMode::Write);
    if (parent == nullptr)
        return false;
    return parent->DeleteFile(Common::FS::GetFilename(path));
}

VirtualDir VfsFilesystem::OpenDirectory(std::string_view path_, VirtualFileOpenMode perms) {
    const auto path = Common::FS::SanitizePath(path_);
    return root->GetDirectoryRelative(path);
}

VirtualDir VfsFilesystem::CreateDirectory(std::string_view path_, VirtualFileOpenMode perms) {
    const auto path = Common::FS::SanitizePath(path_);
...
VirtualDir VfsFilesystem::CopyDirectory(std::string_view old_path_, std::string_view new_path_) {
    ...
    auto old_dir = OpenDirectory(old_path, VirtualFileOpenMode::Read);
    if (old_dir == nullptr)
        return nullptr;
    auto new_dir = OpenDirectory(new_path, VirtualFileOpenMode::Read);
    if (new_dir != nullptr)
        return nullptr;
    new_dir = CreateDirectory(new_path, VirtualFileOpenMode::Write);
    if (new_dir == nullptr)
        return nullptr;
