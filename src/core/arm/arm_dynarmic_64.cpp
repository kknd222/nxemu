...
public:
    ...
    {
        return CheckMemoryAccess(vaddr, 16, CpuDebugWatchpointType::Write) &&
               m_memory.WriteExclusive128(vaddr, value[0], value[1], expected[0], expected[1]);
    }
...
#if defined(__ANDROID__) && (defined(_M_ARM64) || defined(ARCHITECTURE_arm64) || defined(__aarch64__))
    // Android/ARM64 fastmem exclusives can get stuck in guest LDXP/STXP retry loops on some
    // devices/drivers. Keep normal fastmem enabled, but route exclusive writes through the
    // Dynarmic global monitor callbacks for correctness.
    config.fastmem_exclusive_access = false;
    LOG_INFO(Core_ARM, "Android ARM64: disabled fastmem exclusive access");
#endif
