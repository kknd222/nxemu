...
bool Patcher::PatchText(const uint8_t* program_image, uint64_t image_size, uint64_t code_offset, uint64_t code_size) {
    ...
    const auto text = std::span{program_image, static_cast<size_t>(image_size)}.subspan(code_offset, code_size);
    ...
    for (u32 i = ModuleCodeIndex; i < static_cast<u32>(text_words.size()); i++) {
        ...
        if (auto mrs = MRS{inst}; mrs.Verify() && mrs.GetSystemReg() == CntfrqEl0) {
            WriteCntfrqHandler(AddRelocations(), oaknut::XReg{static_cast<int>(mrs.GetRt())});
            continue;
        }
...
bool Patcher::RelocateAndCopy(uint64_t load_base, uint64_t code_offset, uint64_t code_size, uint8_t* program_image, uint32_t* image_size, EntryTrampolines* out_trampolines) {
    ...
    total_program_size -= old_image_size;

    // Only copy to the program image of the last module
    if (m_relocate_module_index == modules.size()) {
        if (this->mode == PatchMode::PreText) {
            ASSERT(old_image_size == total_program_size);
            std::memcpy(program_image, m_patch_instructions.data(),
                        m_patch_instructions.size() * sizeof(u32));
        } else {
            *image_size = static_cast<uint32_t>(old_image_size + patch_size);
            std::memcpy(program_image + old_image_size, m_patch_instructions.data(),
                        m_patch_instructions.size() * sizeof(u32));
        }
        return true;
    }
...
size_t Patcher::GetSectionSize() const noexcept {
    return Common::AlignUp(m_patch_instructions.size() * sizeof(u32), 0x1000);
}
...
void Patcher::WriteSvcTrampoline(ModuleDestLabel module_dest, u32 svc_id) {
    ...
    c.ORR(X0, X0, static_cast<u64>(CpuHaltReason::SupervisorCall));
...
void Patcher::WriteCntfrqHandler(ModuleDestLabel module_dest, oaknut::XReg dest_reg) {
    // CNTFRQ_EL0 is not directly readable by Android userspace on all devices. If left
    // unpatched, guest MRS CNTFRQ_EL0 can terminate NCE with SIGILL. Return the Switch
    // counter frequency expected by HOS instead, matching Eden/yuzu's Android NCE path.
    c.MOV(dest_reg, Common::WallClock::CNTFRQ);
