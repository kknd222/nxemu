...
void PatchCollection::Relocate(int32_t patch_index, uint64_t load_base, uint8_t * program_image, uint32_t * image_size, uint32_t code_offset, uint32_t code_size, uint64_t * segment_addr, uint32_t * segment_size)
{
    ...
    const bool copied_patch_section = patch.RelocateAndCopy(load_base, code_offset, code_size, program_image, image_size, &trampolines);

    // RelocateAndCopy emits the current module's post-SVC trampoline map even when the shared
    // patch section is not copied yet (only the last module in a patcher owns the physical patch
    // bytes). Preserve those per-module handlers so KProcess can resume from SVCs in rtld/main
    // and not re-enter them forever with the generic signal path.
    m_last_post_handlers.reserve(trampolines.size());
    for (const auto & entry : trampolines)
    {
        m_last_post_handlers.emplace_back(entry.first, entry.second);
    }
