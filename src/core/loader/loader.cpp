...
LoaderFileType GuessFromFilename(const std::string & name)
{
    ...
    if (extension == "dnsp" || extension == "nsp")
    {
...
std::string GetFileTypeString(LoaderFileType type)
{
    ...
    case LoaderFileType::XCI:
        return "XCI";
    case LoaderFileType::NAX:
        return "NAX";
    case LoaderFileType::KIP:
        return "KIP";
    case LoaderFileType::DeconstructedRomDirectory:
        return "DeconstructedRomDirectory";
...
std::shared_ptr<AppLoader> GetLoader(Systemloader & loader, FileSys::VirtualFile file, uint64_t program_id, std::size_t program_index)
{
    ...
    if (type == LoaderFileType::Unknown && filename_type != LoaderFileType::Unknown)
    {
        type = filename_type;
    }
